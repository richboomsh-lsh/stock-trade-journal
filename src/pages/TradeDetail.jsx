...
// 시장 배지 스타일 (명세서 8-13 규칙)
function marketBadgeStyle(market) {
  if (market === '코스피') return { bg: '#f0fdf4', color: '#16a34a', border: '#bbf7d0' }
  if (market === '코스닥') return { bg: '#faf5ff', color: '#7c3aed', border: '#e9d5ff' }
...
export default function TradeDetail() {
  ...
  useEffect(() => {
    ...
    ms.current.scale = 1; ms.current.panX = 0; ms.current.panY = 0
  ...
  useEffect(() => {
    ...
    const onStart = (e) => {
      if (e.touches.length === 2) {
        ...
        m.pinchScale = m.scale; m.moving = true; return
      }
      if (e.touches.length !== 1) return
      const t = e.touches[0]
      m.startX = t.clientX; m.startY = t.clientY
      m.panStartX = m.panX; m.panStartY = m.panY
      m.moving = false; m.pinchDist = null
      const now = Date.now()
      if (now - m.lastTap < 300) {
        e.preventDefault()
        if (m.scale > 1) { m.scale = 1; m.panX = 0; m.panY = 0 }
        else              { m.scale = 2 }
        applyTransform(); m.lastTap = 0
      } else { m.lastTap = now }
    }
    ...
    const onMove = (e) => {
      ...
      if (m.scale > 1) { m.panX = m.panStartX + dx; m.panY = m.panStartY + dy; applyTransform() }
    }

    const onEnd = (e) => {
      if (e.touches.length > 0) return
      if (m.pinchDist != null) { m.pinchDist = null; m.moving = false; return }
  ...
  const cardBg = displayRate == null ? '#f8fafc'
    : displayRate > 0 ? '#eff6ff'
    : displayRate < 0 ? '#fef2f2'
    : '#f8fafc'
  const cardBorder = displayRate == null ? '#e2e8f0'
    : displayRate > 0 ? '#bfdbfe'
    : displayRate < 0 ? '#fecaca'
    : '#e2e8f0'
    ...
    <div style={{ padding: isMobile ? '0 0 80px' : '0' }}>
      ...
      <div style={{
        ...
        <button onClick={() => navigate(-1)} style={{
          ...
          fontSize: isMobile ? '16px' : '14px', color: '#475569', whiteSpace: 'nowrap',
        }}>← 뒤로</button>
        <h2 style={{
          fontSize: isMobile ? '18px' : '20px', fontWeight: 700, color: '#1e293b',
        ...
        <Link to={`/edit/${trade.id}`} style={{
          ...
          fontSize: isMobile ? '16px' : '15px', fontWeight: 600, whiteSpace: 'nowrap',
        }}>✏️ 수정</Link>
        <button onClick={deleteTrade} style={{
          padding: '7px 14px', background: '#fee2e2', color: '#dc2626',
          border: '1px solid #fca5a5', borderRadius: '8px',
          fontSize: isMobile ? '16px' : '15px', cursor: 'pointer', fontWeight: 600, whiteSpace: 'nowrap',
        }}>🗑️ 삭제</button>
      </div>
      ...
      <div style={{
        ...
        {hasNetProfit ? (
          /* ✅ 순수익 있을 때: 순수익을 크게 위에, 수익·수수료·세금은 작게 아래에 */
          <>
            ...
            <div style={{
              ...
              {trade.net_profit_rate != null && (
                <div>
                  <div style={{ fontSize: isMobile ? '16px' : '14px', color: '#94a3b8', marginBottom: '4px' }}>순수익률</div>
                  <div style={{ fontSize: isMobile ? '22px' : '26px', fontWeight: 700, color: getProfitColor(trade.net_profit_rate) }}>
              ...
              {trade.net_profit_amount != null && (
                <div>
                  <div style={{ fontSize: isMobile ? '16px' : '14px', color: '#94a3b8', marginBottom: '4px' }}>순수익금</div>
                  <div style={{ fontSize: isMobile ? '18px' : '20px', fontWeight: 700, color: getProfitColor(trade.net_profit_amount) }}>
            ...
            <div style={{
              ...
              {trade.profit_rate != null && (
                <div>
                  ...
                  <div style={{ fontSize: isMobile ? '15px' : '14px', fontWeight: 600, color: getProfitColor(trade.profit_rate) }}>
          ...
          /* 순수익 없을 때: 수익률·수익금·보유기간만 크게 (기존 레이아웃) */
          <div style={{
            display: 'grid',
            gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(3, 1fr)',
            gap: '12px',
          ...
          <div style={{
            ...
            {imageUrls.map((url, i) => (
              <div key={i} onClick={() => setModalIndex(i)} style={{
                ...
                <img src={url} alt={`차트 ${i + 1}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              </div>
            ))}
          </div>
        </Section>
      ...
      {modalIndex !== null && (
        <div ref={modalRef} style={{
          ...
          <div style={{
            display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            padding: '10px 14px', background: 'rgba(0,0,0,0.55)', flexShrink: 0,
          }}>
            <button onClick={closeModal} style={{
              width: 'clamp(26px, 4vmin, 38px)', height: 'clamp(26px, 4vmin, 38px)',
            ...
            {imageUrls.length > 1 ? (
              ...
              }}>{modalIndex + 1} / {imageUrls.length}</span>
            ) : <div />}
          ...
          <div style={{
            ...
            {imageUrls.length > 1 && (
              <button onClick={goPrev} style={{
                ...
                width: 'clamp(28px, 5vmin, 42px)', height: 'clamp(28px, 5vmin, 42px)',
                fontSize: 'clamp(12px, 1.6vmin, 17px)',
                background: modalIndex > 0 ? 'rgba(255,255,255,0.22)' : 'rgba(255,255,255,0.06)',
            ...
            <img ref={imgRef} src={imageUrls[modalIndex]} alt={`차트 ${modalIndex + 1}`}
              draggable={false} style={{
                maxWidth: '88%', maxHeight: '100%', objectFit: 'contain',
                userSelect: 'none', transformOrigin: 'center center',
            ...
            {imageUrls.length > 1 && (
              <button onClick={goNext} style={{
                ...
                width: 'clamp(28px, 5vmin, 42px)', height: 'clamp(28px, 5vmin, 42px)',
                fontSize: 'clamp(12px, 1.6vmin, 17px)',
                background: modalIndex < imageUrls.length - 1 ? 'rgba(255,255,255,0.22)' : 'rgba(255,255,255,0.06)',
                border: 'none', borderRadius: '50%',
          ...
          {imageUrls.length > 1 && (
            <div style={{
              ...
              gap: '8px', padding: '14px 16px', flexShrink: 0,
            }}>
              {imageUrls.map((_, i) => (
                <div key={i} onClick={() => {
                  ms.current.scale = 1; ms.current.panX = 0; ms.current.panY = 0
                  ...
                  width: i === modalIndex ? 'clamp(14px, 2.5vmin, 22px)' : 'clamp(5px, 1vmin, 8px)',
                  height: 'clamp(5px, 1vmin, 8px)', borderRadius: '4px',
                  background: i === modalIndex ? '#fff' : 'rgba(255,255,255,0.32)',
